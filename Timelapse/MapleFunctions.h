...
//兼容私服：优先匹配标准类名 MapleStoryClass；匹配不到时（私服常改窗口类名）
//兜底返回当前进程的主可见顶层窗口，并排除外挂自身的 WinForms 窗体。
static HWND GetMSWindowHandle() {
	HWND msHandle = nullptr;
	HWND fallbackHandle = nullptr;
	TCHAR buf[256] = { 0 };
	ULONG procid;
	...
	for (HWND hwnd = GetTopWindow(nullptr); hwnd != nullptr; hwnd = GetNextWindow(hwnd, GW_HWNDNEXT)) {
		...
		if (procid != curPid) continue;
		if (!GetClassName(hwnd, buf, 256)) continue;
		if (!fallbackHandle && IsWindowVisible(hwnd) && GetWindow(hwnd, GW_OWNER) == nullptr
			&& _tcsncmp(buf, _T("WindowsForms"), 12) != 0) {
			fallbackHandle = hwnd;
		}
	...
	return msHandle ? msHandle : fallbackHandle;
}
